...
if ( ! function_exists( 'alg_get_current_user_first_role' ) ) {
	...
	function alg_get_current_user_first_role() {
        global $woocommerce;
        // If getting price within admin order screens, we need to get the role of customer the order is for, not current user
        if(isset($_GET['action'])){
            if($_GET['action'] == 'woocommerce_json_search_products_and_variations'){
        ...
        if(isset($order_id) && wc_get_order($order_id)){
            ...
            $current_user = get_user_by('id', $order->get_user_id());
        } else {
            $current_user = wp_get_current_user();
        }
        //var_dump($current_user);
		foreach ($current_user->roles as $key => $value) {
			// Intentionally rewritting over the same variable...all we need is the last value in the array
			$secondary_role = $value;
		}
		return ( isset( $secondary_role ) && '' !== $secondary_role ) ? $secondary_role : 'guest';
	}
}
