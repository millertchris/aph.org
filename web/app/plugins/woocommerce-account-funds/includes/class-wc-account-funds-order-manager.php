...
class WC_Account_Funds_Order_Manager {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_admin_order_totals_after_total', array( $this, 'add_funds_used_after_order_total' ), 10 );
	...
	public function remove_funds_from_recalculation( $and_taxes, $order ) {
		$_funds_used = $order->get_meta( '_funds_used' );
	...
	public function maybe_remove_funds( $order_id ) {
		...
		if ( $customer_id && ! $order->get_meta( '_funds_removed' ) ) {
			if ( $funds = $order->get_meta( '_funds_used' ) ) {
				WC_Account_Funds::remove_funds( $customer_id, $funds );
				$order->add_order_note( sprintf( __( 'Removed %s funds from user #%d', 'woocommerce-account-funds' ), wc_price( $funds ), $customer_id ) );
			}
	...
	public function woocommerce_checkout_update_order_meta( $order_id, $posted ) {
		if ( $posted['payment_method'] !== 'accountfunds' && WC_Account_Funds_Cart_Manager::using_funds() ) {
			...
			$order->add_meta_data( '_funds_removed', 0 );
			$order->save();
		}
	}
	...
	public function maybe_increase_funds( $order_id ) {
		...
		if ( $customer_id && ! $order->get_meta( '_funds_deposited' ) ) {
	...
	public function woocommerce_get_order_item_totals( $rows, $order ) {
		if ( $_funds_used = $order->get_meta( '_funds_used' ) ) {
	...
	public static function adjust_total_to_include_funds( $total, $order ) {
		...
		// 07/27/2019 Changed to accomodate non 0 order total
	...
	public function add_funds_used_after_order_total( $order_id ) {
		$order = wc_get_order( $order_id );
		$funds_used = floatval( $order->get_meta( '_funds_used' ) );
		if ( $funds_used <= 0 ) {
			return;
		}
		...
		<!-- 07/27/2019 Not needed because the site doesnt support partial account funds payments -->
		<!-- <tr>
