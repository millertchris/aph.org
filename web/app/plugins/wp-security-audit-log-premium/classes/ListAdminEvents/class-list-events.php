		...
		public static function format_column_value( $item, $column_name ) {
			switch ( $column_name ) {
				...
				case 'user':
					...
					if ( $user instanceof \WP_User ) {
						...
						if ( \class_exists( 'WSAL_SearchExtension' ) ) {
							...
							$uhtml = '<a class="search-user" data-darktooltip="' . esc_attr( $tooltip ) . '" data-user="' . $user->user_login . '" data-user-email="' . $user->data->user_email . '" href="' . $user_edit_link . '" target="_blank">' . esc_html( $display_name ) . '</a>';
						}
						// @premium:end
				...
				case 'data':
					...
					// @premium:start
					$btns .= ' ' . Event_Notes::render_add_note_to_event_trigger( $item );
					$btns .= ' ' . Copy_Event_Data::render_copy_event_data_triggers( $item );
					// @premium:end
			...
			if ( count( $this->items ) > 0 && count( self::$query_occ ) > 0 ) {
				...
				<script>
					document.addEventListener("readystatechange", event => { 
						...
						// Only process 300 records at a time, as wsal_export_csv_results can be resource-intensive.
						const records = 300;
						...
						jQuery.ajax({
								...
								success: function(data) {
									/**
									 * Server returns step already incremented, no need to increment it here
									 * ! If wsal_export_csv_results returns step 0 it means that the processing is finished!
									 */
									let returnedStep = parseInt(data.data.step, 10);

									// Processed records count, only if step > 0.
									let processed = returnedStep > 0 ? data.data.record_count : 0;
									
									if (processed >= count) {
										processed = count
									};

									let percent = count > 0 ? Math.round((processed / count) * 100) : 100;

									if (percent > 100) {
										percent = 100
									};

									// Animate progress of loading bar.
									jQuery('.progress-bar').css('transform', 'translate('+percent+'%)');

									/**
									 * Finish processing CSV when server-side code sends step === 0 or processed records is higher than count
									 */
									if (returnedStep === 0 || processed >= count) {
										finished_csv_exporting(elmnt, data);
									} else {
										// ...or recall the function to process the next step.
										wsal_export_csv_results(returnedStep, nonce, query, order, count, elmnt);
									}
								},
								dataType: "json"
								});
