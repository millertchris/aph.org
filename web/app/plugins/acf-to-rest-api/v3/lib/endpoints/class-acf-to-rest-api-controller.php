...
if ( ! class_exists( 'ACF_To_REST_API_Controller' ) ) {
	class ACF_To_REST_API_Controller extends WP_REST_Controller {
		...
		/**
		 * Check if public access is explicitly allowed via constant.
		 *
		 * @since 3.3.5
		 * @return bool
		 */
		protected function allow_public_access() {
			return defined( 'ACF_TO_REST_API_ALLOW_PUBLIC_ACCESS' ) && ACF_TO_REST_API_ALLOW_PUBLIC_ACCESS;
		}

		/**
		 * Default permission check for reading items.
		 * Requires user to be logged in unless public access is explicitly allowed.
		 *
		 * @since 3.3.5
		 * @return bool
		 */
		...
		public function register_field_callback( $data ) {
			// Security fix: Check permissions before exposing ACF data via register_rest_field
		...
		public function get_items_permissions_check( $request ) {
			return apply_filters( 'acf/rest_api/items_permissions/get', $this->check_read_permission(), $request, $this->type );
		}
