 ...
 */
function wo_public_get_access_token( $access_token ) {
	...
	$results = $wpdb->get_row(
		$wpdb->prepare( "SELECT * FROM {$wpdb->prefix}oauth_access_tokens WHERE access_token = %s", array( $access_token ) ), ARRAY_A
	);
	
	if ( $results ) {
		$expires = strtotime( $results['expires'] );
