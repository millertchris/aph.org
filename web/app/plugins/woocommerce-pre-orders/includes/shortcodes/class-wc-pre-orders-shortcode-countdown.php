...
class WC_Pre_Orders_Shortcode_Countdown {
	...
	private static function enqueue_scripts() {
		...
		wp_enqueue_script( 'jquery-countdown', $wc_pre_orders->get_plugin_url() . '/build/jquery-countdown/jquery.countdown.js', array( 'jquery' ), '1.6.1', true );
		...
		if ( defined( 'WPLANG' ) && WPLANG ) {
			...
			if ( ! is_readable( $wc_pre_orders->get_plugin_path() . '/build/jquery-countdown/jquery.countdown-' . $localization . '.js' ) ) {
				$localization = $lang;
				if ( ! is_readable( $wc_pre_orders->get_plugin_path() . '/build/jquery-countdown/jquery.countdown-' . $localization . '.js' ) ) {  // try falling back to base language if dialect is not found
					$localization = null;
				}
			}

			if ( $localization ) {
				wp_enqueue_script( 'jquery-countdown-' . $localization, $wc_pre_orders->get_plugin_url() . '/build/jquery-countdown/jquery.countdown-' . $localization . '.js', array( 'jquery-countdown' ), '1.6.1', true );
			}
		}
	}
}
