...
class Templates
{
    static function cart_preview_content() {
        ...
        echo '<ul class="ajax-sub-menu sub-menu cart right">';
            ...
            echo '</li>';
            echo '<li class="view-cart"><a role="button" href="/cart/" class="btn white">View Cart</a></li>';
            echo '<li class="checkout"><a role="button" href="/checkout/" class="btn white">' . $orderText . '</a></li>';
    ...
    static function cart_preview_link(){
        ...
        echo '<a role="button" class="cart-preview-link" href="#">';
            ...
            <section class="layout hero align-right category-hero" name="hero">
                <div class="wrapper">
                    <div class="row">
                        ...
                        <div class="image">
                            <img src="<?php echo $shop_hero_bg; ?>" alt="APH shop featured image of a student using a magnifier to read their school work."/>
                        </div>
                    </div>
                </div>
            </section>
    ...
    static function update_strings($translated, $untranslated, $domain){
        ...
        if(is_user_role('copy_of_csr') && $domain === 'woocommerce'){         
            switch ($translated){
    ...
    static function display_fq_accounts($current_user) {
        foreach (wp_get_terms_for_user($current_user, 'user-group') as $group) {
			...
			$yearAllocations = get_field('fq_yearAllocations', 'user-group_' . $group->term_id);
            ...
            if (Roles::userHas([Roles::EOT, Roles::OOA], $current_user)) {
				...
				echo '<li>FQ allocation for the year: ' . wc_price( $yearAllocations ) . '</li>';

                echo "<li><a role='button' href='#users-{$group->term_id}'>View Users Access</a></li>";

            } elseif (Roles::userHas(Roles::TVI, $current_user)) {

                echo "<li><a role='button' href='#users-{$group->term_id}'>View Contact Info</a></li>";
    ...
    static function facetwp_pager_html($html, $params){
        $html = '<nav aria-label="Search Pagination">' . $html . '</nav>';
        return $html;
    }

    static function event_date_html($row){

        $is_all_day = $row["ad"];
        $show_end_date = true;
        if($is_all_day){
            $start_date_time = date_format(date_create($row["sdt"]), 'F j, Y');
            $end_date_time = date_format(date_create($row["edt"]), 'F j, Y');
            if($start_date_time == $end_date_time){
                $show_end_date = false;
            }
        } else {
            $start_date = date_format(date_create($row["sdt"]), 'F j, Y');
            $end_date = date_format(date_create($row["edt"]), 'F j, Y');
            $start_date_time = date_format(date_create($row["sdt"]), 'F j, Y, g:ia');
            $end_date_time = date_format(date_create($row["edt"]), 'F j, Y, g:ia');
            if($start_date == $end_date && $start_date_time != $end_date_time){
                $show_end_date = true;
                $end_date_time = date_format(date_create($row["edt"]), 'g:ia');
            }                                
            if($start_date_time == $end_date_time){
                $show_end_date = false;
            }                                
        ...
        <?php if ($start_date_time && $end_date_time) : ?>
            ...
            <p class="occurrence">
                <?php
                    ...
                    echo ($show_end_date) ? $start_date_time . ' - ' . $end_date_time : $start_date_time;
