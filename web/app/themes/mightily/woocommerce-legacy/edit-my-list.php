...
<?php do_action( 'woocommerce_wishlists_before_wrapper' ); ?>
    <div id="wl-wrapper" class="product woocommerce"> <!-- product class so woocommerce stuff gets applied in tabs -->
        ...
        <div class="wl-intro">
            ...
            <?php endif; ?>
            <?php if(!isset($_GET['settings'])) : ?>
                <a href="/my-account/account-wishlists/">My Wishlists</a> | 
            <?php endif; ?>
            <?php if(isset($_GET['settings'])) : ?>
                <a href="?wlid=<?php echo $wishlist->id;?>">Back to List</a> | 
            <?php endif; ?>            
            <a class="wlconfirm" data-message="<?php _e( 'Are you sure you want to delete this list?', 'wc_wishlist' ); ?>" href="<?php $wishlist->the_url_delete(); ?>"><?php _e( 'Delete List', 'wc_wishlist' ); ?></a><?php if(!isset($_GET['settings'])) : ?> | <?php endif; ?>
            <?php if(!isset($_GET['settings'])) : ?>
                <a href="?wlid=<?php echo $wishlist->id;?>&settings=1">Edit List</a>
            <?php endif; ?>
            </p>
        </div>
        <?php if(!isset($_GET['settings'])) : ?>
				...
				<?php if ( sizeof( $wishlist_items ) > 0 ) : ?>
                    <form action="<?php $wishlist->the_url_edit(); ?>" method="post" class="wl-form" id="wl-items-form">
                        ...
                        <table class="shop_table cart wl-table manage" cellspacing="0">
							...
							foreach ( $wishlist_items as $wishlist_item_key => $item ) {
								...
								if ( $_product->exists() && $item['quantity'] > 0 ) {
                                    ...
                                    <tr class="cart_table_item">
                                        ...
                                        <a rel="nofollow" href="<?php echo woocommerce_wishlist_url_item_add_to_cart( $wishlist->id, $wishlist_item_key, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false, 'edit' ); ?>" class="wishlist-add-to-cart-button btn alt"><?php _e( 'Add to Cart', 'wc_wishlist' ); ?></a>
                                            <?php elseif ( $_product->get_type() == 'external' ) : ?>
                                        <a rel="nofollow" href="<?php echo esc_url( $_product->add_to_cart_url() ); ?>" rel="nofollow" class="single_add_to_cart_button btn alt"><?php echo esc_html( $_product->single_add_to_cart_text() ); ?></a>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                        <a rel="nofollow" href="<?php echo woocommerce_wishlist_url_item_remove( $wishlist->id, $wishlist_item_key ); ?>" class="wlconfirm btn alt" title="<?php _e( 'Remove this item from your wishlist', 'wc_wishlist' ); ?>" data-message="<?php esc_attr( _e( 'Are you sure you would like to remove this item from your list?', 'wc_wishlist' ) ); ?>">Delete</a>
                            ...
                            <tr>
                                <td colspan="4"></td>
                                <td><a rel="nofollow" href="<?php echo woocommerce_wishlist_url_add_all_to_cart( $wishlist->id, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="btn alt wl-add-all"><?php _e( 'Add All To Cart', 'wc_wishlist' ); ?></a></td>
                            </tr>
                            </tbody>
                        </table>
