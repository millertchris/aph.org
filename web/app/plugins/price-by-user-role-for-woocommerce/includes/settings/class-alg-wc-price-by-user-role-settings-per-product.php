...
//			}
		...
		public function save_meta_box( $post_id, $post ) {
			...
			foreach ( $this->get_meta_box_options() as $option ) {
				...
				if ( $is_enabled ) {
					...
					$product = wc_get_product( $the_post_id );
					if ( $product ) {
						$meta_value = apply_filters( 'alg_wc_price_by_user_role_save_meta_box_value', $option_value, $option['name'] );
						$product->update_meta_data( $the_meta_name, $meta_value );
						$product->save();
					}
				}
			}
		...
		public function create_meta_box() {
			...
			$prices_enabled = $product ? $product->get_meta( '_' . $feature_enabled['name'], true ) : $feature_enabled['default'];
			$prices_enabled = $prices_enabled ? $prices_enabled : $feature_enabled['default'];
			$options        = '';
			...
			foreach ( $options_data as $option ) {
				...
				if ( $is_enabled ) {
					if ( 'title' === $option['type'] ) {
						...
						$the_meta_name     = ( isset( $option['meta_name'] ) ) ? $option['meta_name'] : '_' . $option['name'];
						$the_product = wc_get_product( $the_post_id );
						if ( $the_product ) {
							$option_value = $the_product->get_meta( $the_meta_name, true );
							if ( ! $option_value ) {
