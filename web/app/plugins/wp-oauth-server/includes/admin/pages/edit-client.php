...
function wo_admin_edit_client_page() {
    ...
    <div class="wrap">
        ...
        <h2><?php _e( 'Edit Client', 'wp-oauth' ); ?>
            <small>( id: <?php echo sanitize_text_field( $client->ID ); ?> )</small>
            <a class="add-new-h2 "
		...
		<?php if ( ! is_null( $message ) ) : ?>
            <div class="notice notice-success is-dismissible">
                <p><?php echo sanitize_text_field( $message ); ?></p>
            </div>
		<?php endif; ?>
        ...
        <form class="wo-form" action="" method="post">
            ...
            <input type="hidden" name="edit_client" value="<?php echo sanitize_text_field( $client->ID ); ?>"/>
            <input type="hidden" name="post_id" value="<?php echo sanitize_text_field( $client->ID ); ?>"/>
                    ...
                    <label class="checkbox-grid"> Allowed Grant Types
                        ...
                        <label> <strong>Implicit</strong>
                            <input type="checkbox" name="grant_types[]"
                                   value="implicit"
								<?php
								if ( in_array( 'implicit', $client->grant_types ) ) {
									echo ' checked';
								}
								?>
                            />
                        ...
                        <label> <strong>Refresh Token</strong>
                            <input type="checkbox" name="grant_types[]"
                                   value="refresh_token"
								<?php
								if ( in_array( 'refresh_token', $client->grant_types ) ) {
									echo ' checked';
								}
								?>
                            />
                            ...
                            <div class="col span_6_of_6">
                                <label> Client Name
                                    <input class="emuv-input" type="text" name="name"
                                           value="<?php echo sanitize_text_field( $client->post_title ); ?>" required/>
                                </label>

                                <label> Redirect URI
                                    <input class="emuv-input" type="text" name="redirect_uri"
                                           value="<?php echo sanitize_text_field( get_post_meta( $client->ID, 'redirect_uri', true ) ); ?>"/>
                                </label>
                                ...
                                <label> Client ID
                                    <input class="emuv-input" type="text" name="client_id"
                                           value="<?php echo sanitize_text_field( get_post_meta( $client->ID, 'client_id', true ) ); ?>"/>
                                </label>

                                <label> Client Secret
                                    <input class="emuv-input" type="text" name="client_secret"
                                           value="<?php echo sanitize_text_field( get_post_meta( $client->ID, 'client_secret', true ) ); ?>"/>
                                </label>
                                ...
                                <div style="margin-top: 2.5em" class="advanced-options">
                                    ...
                                    <label> Client Scope(s)
                                        ...
                                        <input class="emuv-input" type="text" name="scope"
                                               value="<?php echo sanitize_text_field( get_post_meta( $client->ID, 'scope', true ) ); ?>"
                                               placeholder="basic"/>
                                    </label>
                                </div>
                            </div>
