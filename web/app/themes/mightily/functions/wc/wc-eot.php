...
function hide_wc_payment_methods() {
    if (is_user_logged_in()) {
        ...
        if (in_array('eot', $roles) || in_array('eot-assistant', $roles) || in_array('net', $roles)) : ?>
<script>
jQuery(function() {
...
function hide_credit_card_payment_gateway($available_gateways) {
    ...
    if (is_user_logged_in()) {
        ...
        if (in_array('eot', $roles) || in_array('eot-assistant', $roles) || in_array('teacher', $roles) || in_array('net', $roles)) {
            // Remove credit card
...
function allow_backorder_override($product) {
    ...
    if ($product->get_attribute('federal-quota-funds') && $product->get_attribute('federal-quota-funds') == 'Available') {
        ...
        if (isset($current_user->roles) && is_array($current_user->roles)) {
            ...
            if(defined('DOING_AJAX') && DOING_AJAX && in_array('copy_of_csr', $current_user->roles)){
                return false;
            }
            // Check user is eot, eota, teacher or csr
            if (in_array('eot', $current_user->roles) || in_array('eot-assistant', $current_user->roles) || in_array('teacher', $current_user->roles) || in_array('copy_of_csr', $current_user->roles) || in_array('net', $current_user->roles)) {
