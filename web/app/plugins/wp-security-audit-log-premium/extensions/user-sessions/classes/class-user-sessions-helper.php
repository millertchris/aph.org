...
if ( ! class_exists( '\WSAL\Helpers\User_Sessions_Helper' ) ) {
	...
	class User_Sessions_Helper {
		...
		public static function is_idle_session_cleanup_enabled() {
			...
			$enabled = Settings_Helper::get_option_value( 'wsal_usersessions_policies_enabled', false );
			if ( ! $enabled ) {
				return false;
			}
		...
		public static function get_master_sessions_policy() {
			...
			$policy   = Settings_Helper::get_option_value( 'wsal_usersessions_policy', array() );
		...
		public static function get_role_sessions_policy( $role = '' ) {
			...
			$policy = Settings_Helper::get_option_value( "wsal_usersessions_policy_{$role}" );
