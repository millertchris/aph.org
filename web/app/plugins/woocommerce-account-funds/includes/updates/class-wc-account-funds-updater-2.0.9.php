...
class WC_Account_Funds_Updater_2_0_9 implements WC_Account_Funds_Updater {
	...
	public function update() {
		...
		foreach ( $orders as $order_post ) {
			...
			$recurring_total = $this->_get_recurring_total( $order_post->post_parent );
			$order_total     = $order->get_meta( '_order_total' );

			if ( $order_total > 0 ) {
				$order->update_meta_data( '_order_total', 0 );
			}
	...
	private function _get_recurring_total( $parent_id ) {
		...
		if ( version_compare( WC_Subscriptions::$version, '2.0.0', '<' ) ) {
			$recurring_total = $parent_order->get_meta( '_order_recurring_total' );
		} else {
			$recurring_total = $parent_order->get_meta( '_wcs_migrated_order_recurring_total' );
		}
