...
class WC_Pre_Orders_Checkout {
	...
	public function add_release_date_message( $message, $order ): string {
		...
		$pre_order_product = WC_Pre_Orders_Order::get_pre_order_product( $order );
		...
		$charge_upon_release = WC_Pre_Orders_Product::product_is_charged_upon_release( $pre_order_product );
		if ( ! $charge_upon_release ) {
			return $message;
		}

		$timestamp         = WC_Pre_Orders_Product::get_localized_availability_datetime_timestamp( $pre_order_product );
		$availability_date = WC_Pre_Orders_Product::get_localized_availability_date( $pre_order_product );
		...
		if ( WC_Pre_Orders_Order::order_has_payment_token( $order ) ) {
			...
			$message_template = $timestamp
				...
				? __( 'Your pre-order has been received. You will be automatically charged for your order via your selected payment method when your pre-order is released on %s. Your order details are shown below for your reference.', 'woocommerce-pre-orders' )
				: __( 'Your pre-order has been received. You will be automatically charged for your order via your selected payment method when your pre-order is released. Your order details are shown below for your reference.', 'woocommerce-pre-orders' );
		} else {
			// Message for manual payment upon release.
			$message_template = $timestamp
				/* translators: %s is the availability date for the pre-order */
				? __( 'Your pre-order has been received. You will be prompted for payment for your order when your pre-order is released on %s. Your order details are shown below for your reference.', 'woocommerce-pre-orders' )
				: __( 'Your pre-order has been received. You will be prompted for payment for your order when your pre-order is released. Your order details are shown below for your reference.', 'woocommerce-pre-orders' );
		}
