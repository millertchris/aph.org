...
class Products {
    ...
    static function jsonSearchProductsFilter( $products ) {
        ...
        $products        = array();
        $has_private_access  = Roles::userHas([Roles::ADM, Roles::CSR], wp_get_current_user());
        ...
        foreach ( $product_objects as $product_object ) {
            ...
            $is_repair = has_term( 'Repair', 'product_cat', $product_object->get_id());
            ...
            if($show && !get_field('discontinued', $product_object->get_id())){
...
//        aph_write_log($request, 'XX3 shop_order request');
    ...
    static function is_purchasable($is_purchasable, $product){
        if (class_exists('WC_Pre_Orders_Product') && \WC_Pre_Orders_Product::product_can_be_pre_ordered($product)){
            if (is_user_logged_in() || isset($_SESSION['tmp_usr_get_cart'])) {
                $user = wp_get_current_user();
                $roles = $user->roles;
                if (isset($_SESSION['tmp_usr_get_cart']) || in_array('copy_of_csr', $roles) || in_array('eot', $roles) || in_array('eot-assistant', $roles) || in_array('teacher', $roles) || in_array('administrator', $roles)) {
                    $is_purchasable = true;
                } else {
    ...
    static function get_cart_count() {
        if (parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH) == '/cart-count') {
            $cart_count = WC()->cart->get_cart_contents_count();
            $cart_skus = [];
            foreach ( WC()->cart->get_cart() as $cart_item ) {
                $cart_skus[] = $cart_item['data']->get_sku();
            }
            echo json_encode(
                [
    ...
    static function add_to_cart_by_id() {
        if (parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH) == '/add-to-cart-by-id') {
            // var_dump(WC()->cart->get_cart_contents_count());
            $product_id = $_GET['id'] ? sanitize_text_field($_GET['id']) : false;
            $quantity = $_GET['quantity'] ? sanitize_text_field($_GET['quantity']) : 1;
            if($product_id && WC()->cart->get_cart_contents_count() && WC()->cart->add_to_cart($product_id, $quantity)){
    ...
    static function add_to_cart_by_catalog_number() {
        if (parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH) == '/add-to-cart-by-catalog-number') {
            $product_catalog_number = $_GET['catalog-number'] ? sanitize_text_field($_GET['catalog-number']) : false;
            if(!$product_catalog_number){
                echo json_encode(false);
                exit;
            }
            $product_id = wc_get_product_id_by_sku($product_catalog_number);
            $quantity = $_GET['quantity'] ? sanitize_text_field($_GET['quantity']) : 1;
            if($product_id && WC()->cart->get_cart_contents_count() && WC()->cart->add_to_cart($product_id, $quantity)){
                echo json_encode(true);
            } else {
