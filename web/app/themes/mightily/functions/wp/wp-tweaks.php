<?php
    ...
    function custom_mime_types( $mimes ) {
		...
		$mimes['ebrl'] = 'application/octet-stream';
        return $mimes;
    }
    ...
    function all_login_redirect($redirect, $user) {       
        ...
        if (isset($_GET['redirect_to']) && strpos($_GET['redirect_to'], 'accept-eot-invitation') !== false) {
            return $_GET['redirect_to'];
        }        
        if ($redirect == home_url('/checkout/') || $redirect == '/checkout/') {
            return $redirect;
        }
    ...
    function filter_woocommerce_login_redirect( $redirect, $user ) { 
        ...
        if( $redirect == home_url('/checkout/') || $redirect == '/checkout/' ) {
            ...
            $redirect = $redirect;
        } elseif(strpos($redirect, 'accept-eot-invitation') !== false){
            $redirect = $redirect;
        } else {
    ...
    add_filter( 'facetwp_facet_render_args', function( $args ) {
        ...
        if ($args['facet']['name'] == 'content_types') {
            foreach($args['values'] as $facetkey => $facetvalue){
                ...
                if($facetvalue['facet_display_value'] == 'WooCommerce Products'){
                    $args['values'][$facetkey]['facet_display_value'] = 'Shop';
                }                                
