            ...
            if(data == 'FALSE'){
                ...
                jQuery.each(JSON.parse(data), function( indexa, valuea ) {
                    ...
                    if(valuea['Error'] == ''){
                        ...
                        dataHtml += '<span>Account Balance: $' + valuea['fq_account_balance'] + '</span><br />';
                        dataHtml += '<span>Outstanding Order Balance: $' + valuea['fq_outstanding_balance'] + '</span><br />';
                        dataHtml += '<strong>Current Available Funding:</strong><br />';
                        dataHtml += '<span class="h4">$' + valuea['fq_available_funding'] + '</span><br />';
                        dataHtml += '<span>Overspent towards next fiscal year: $' + valuea['fq_overspent'] + '</span>';
                        dataHtml += '</div>';
