	...
	public static function extend_cart_item_data( $cart_item ) {
		...
		if ( \WC_Pre_Orders_Product::product_can_be_pre_ordered( $product->get_id() ) ) {
			$item_data = array(
				...
				'unformatted_total'    => \WC_Pre_Orders_Product::product_is_charged_upon_release( $product )
				? get_option( 'wc_pre_orders_upon_release_order_total_format' )
				: get_option( 'wc_pre_orders_upfront_order_total_format' ),
			);
		}
