...
jQuery(document).ready(function () {
	...
	jQuery( document ).on( 'click', '#do-session-bulk-action', function ( event ) {
		...
		jQuery( '.bulk-checkbox:checked').each(function( index ) {
			UserIDs.push( jQuery( this ).attr('id') );
		});
		...
		if ( UserIDs.length > 0 && Action == 'terminate-sessions' ) {
			...
			jQuery( UserIDs ).each(function( index, value ) {
				var session_data = {
					action: 'destroy_session',
					user_id: jQuery( '.bulk-checkbox[id="'+value+'"]' ).val(),
					token: value,
					wpnonce: jQuery( '.bulk-checkbox[id="'+value+'"]' ).data( 'bulk-destroy-nonce' ),
				}
