...
class WC_Pre_Orders_Admin_Pre_Orders {
	...
	public function process_cancel_pre_order_action() {
		...
		$message = sprintf( __( 'Pre-order #%s cancelled.', 'woocommerce-pre-orders' ), $order_id );
		if ( ! WC_Pre_Orders_Order::order_will_be_charged_upon_release( $order_id ) ) {
			$message .= ' ';
			$message .= __( 'The order was paid upfront, you will need to manually process a refund for this order.', 'woocommerce-pre-orders' );
		}
	...
	private function show_actions_tab() {
		...
		$has_pre_order_products = WC_Pre_Orders_Manager::has_pre_order_enabled_products();
	...
	private function show_manage_tab() {
		...
		if ( isset( $_GET['s'] ) && sanitize_text_field( wp_unslash( $_GET['s'] ) ) ) {
			...
			echo '<span class="subtitle">' . sprintf( esc_html__( 'Search results for "%s"', 'woocommerce-pre-orders' ), esc_attr( $search_query ) ) . '</span>'; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		}
		echo '</h2>';
		...
		$fields   = array(
			...
			'email'       => array(
				...
				array(
					...
					'desc' => sprintf( __( 'You may send an email message to all customers who have pre-ordered a specific product. This will use the default template specified for the %1$sCustomer Note%2$s Email.', 'woocommerce-pre-orders' ), '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=email&section=wc_email_customer_note' ) . '">', '</a>' ),
					'type' => 'title',
				),
				...
				array(
					...
					'custom_attributes' => array(
						'required'         => 'required',
						'data-action'      => 'woocommerce_json_search_pre_order_enabled_products',
						'data-placeholder' => __( 'Search for a product&hellip;', 'woocommerce-pre-orders' ),
					),
				),
