...
// Mightily
const addProductEvent = new CustomEvent("addProductEvent", {
	detail: {},
	bubbles: true,
	cancelable: true,
	composed: false,
...
// Mightily
const addShippingEvent = new CustomEvent("addShippingEvent", {
	detail: {},
	bubbles: true,
	cancelable: true,
	composed: false,
...
// Mightily
const deleteItemEvent = new CustomEvent("deleteItemEvent", {
	detail: {},
	bubbles: true,
	cancelable: true,
	composed: false,
...
// Mightily
const addTaxEvent = new CustomEvent("addTaxEvent", {
	detail: {},
	bubbles: true,
	cancelable: true,
	composed: false,
...
// Mightily
const deleteTaxEvent = new CustomEvent("deleteTaxEvent", {
	detail: {},
	bubbles: true,
	cancelable: true,
	composed: false,
		...
		add_shipping: function() {
			...
			$.post( woocommerce_admin_meta_boxes.ajax_url, data, function( response ) {
				if ( response.success ) {
					...
					// Mightily
					document.querySelector("#woocommerce-order-items").dispatchEvent(addShippingEvent);					
				} else {
					window.alert( response.data.error );
				}
				wc_meta_boxes_order_items.unblock();
			...
			if ( answer ) {
				...
				$.ajax({
					...
					complete: function() {
						...
						// Mightily
						document.querySelector("#woocommerce-order-items").dispatchEvent(deleteItemEvent);
					}
				});
			}
			return false;
		...
		delete_tax: function() {
			if ( window.confirm( woocommerce_admin_meta_boxes.i18n_delete_tax ) ) {
				...
				$.ajax({
					...
					// Mightily
					document.querySelector("#woocommerce-order-items").dispatchEvent(deleteTaxEvent);							
					}
				});
			} else {
		...
		backbone: {
			...
			response: function( e, target, data ) {
				if ( 'wc-modal-add-tax' === target ) {
					...
					// Mightily
					document.querySelector("#woocommerce-order-items").dispatchEvent(addTaxEvent);
				}
				if ( 'wc-modal-add-products' === target ) {
					// Build array of data.
			...
			add_items: function( add_items ) {
				...
				$.ajax({
					...
					complete: function() {
						...
						// Mightily
						document.querySelector("#woocommerce-order-items").dispatchEvent(addProductEvent);
