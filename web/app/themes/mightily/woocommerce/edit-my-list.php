...
<?php do_action( 'woocommerce_wishlists_before_wrapper' ); ?>
    <div id="wl-wrapper" class="product woocommerce"> <!-- product class so woocommerce stuff gets applied in tabs -->
        ...
        <div class="wl-intro">
            ...
            <p>            
            <!-- <a href="/my-lists/create-a-list/">Create a list</a> -->
            <?php if(!isset($_GET['settings'])) : ?>
                <?php if ( $sharing == 'Public' || $sharing == 'Shared' ) : ?>
                    <?php if ( $wishlist_items && count( $wishlist_items ) ) : ?>
                        <a rel="nofollow" class="wl-email-button" href="#share-via-email-<?php echo $wishlist->id; ?>"><?php _e( 'Email List', 'wc_wishlist' ); ?></a> | 
                    <?php endif; ?>
                <?php endif; ?>
            <?php endif; ?>
            <?php if(!isset($_GET['settings'])) : ?>
                <a role="button" href="/my-account/account-wishlists/">My Wishlists</a> | 
            <?php endif; ?>
            <?php if(isset($_GET['settings'])) : ?>
                <a role="button" role="button" href="?wlid=<?php echo $wishlist->id;?>">Back to List</a> | 
            <?php endif; ?>            
            <a role="button" class="wlconfirm" data-message="<?php _e( 'Are you sure you want to delete this list?', 'wc_wishlist' ); ?>" href="<?php $wishlist->the_url_delete(); ?>"><?php _e( 'Delete List', 'wc_wishlist' ); ?></a><?php if(!isset($_GET['settings'])) : ?> | <?php endif; ?>
            <?php if(!isset($_GET['settings'])) : ?>
                <a role="button" href="?wlid=<?php echo $wishlist->id;?>&settings=1">Edit List</a>
            <?php endif; ?>
            </p>
        </div>
        <?php if(!isset($_GET['settings'])) : ?>
        ...
        <div class="wl-tab-wrap">
            ...
            <div class="wl-panel panel">
				<?php if ( sizeof( $wishlist_items ) > 0 ) : ?>
                    <form action="<?php $wishlist->the_url_edit(); ?>" method="post" class="wl-form" id="wl-items-form">
                        ...
                        <table class="shop_table cart wl-table manage" cellspacing="0">
							...
							foreach ( $wishlist_items as $wishlist_item_key => $item ) {
								...
								if ( $_product->exists() && $item['quantity'] > 0 ) {
                                    ...
                                    <tr class="cart_table_item">
                                        ...
                                        <span class="select-title td-titles desktop-hide">Select</span>
                                        ...
                                        <span class="product-title td-titles desktop-hide">Product</span>
                                        <?php
                                        if ( ! $product_permalink ) {
                                        ...
                                        $fqProduct = $_product->get_attribute( 'federal-quota-funds' );
                                        if($fqProduct == 'Available'){
                                            echo '<p>Federal Quota Eligible</p>';
                                        }
                                        if($_product->get_shipping_class()){
                                            $shipping_class = $_product->get_shipping_class();
                                            $shipping_class_name = get_term_by('slug', $shipping_class, 'product_shipping_class');
                                            echo '<p>Shipping: '.$shipping_class_name->name.'</p>';
                                        }
                                        ...
                                        <span class="price-title td-titles desktop-hide">Price</span>
                                            <?php
                                        ...
                                        <span class="quantity-title td-titles desktop-hide">Quantity</span>
                                            <?php
                                            if ( $_product->is_sold_individually() ) {
                                                ...
                                                $quantity_query = array(
                                                    'url' => '/my-lists/edit-my-list/?wlid='.$wishlist->id,
                                                    'wlid' => $wishlist->id,
                                                    'wlaction' => 'manage-list',
                                                    'wlitem' => array($wishlist_item_key),
                                                    'wlmovetarget' => '0',
                                                    'wlupdateaction' => 'quantity',
                                                    'wleditaction2' => 'quantity',
                                                    'cart' => array(
                                                        $wishlist_item_key => array(
                                                            'qty' => '4'
                                                        )
                                                    ),
                                                    '_n' => wp_create_nonce( 'wc-wishlists-manage-list')
                                                );
                                                //var_dump($quantity_query);
                                            ...
                                            <div class="quantity">
                                                ...
                                                <img class="quantity-input-loader" src="<?php echo get_bloginfo('template_directory'); ?>/app/assets/img/loader.gif" alt="Loading Icon"/>
                                            </div>
                                        </td>
                                        ...
                                        <a role="button" rel="nofollow" href="<?php echo woocommerce_wishlist_url_item_add_to_cart( $wishlist->id, $wishlist_item_key, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false, 'edit' ); ?>" class="wishlist-add-to-cart-button btn alt"><?php _e( 'Add to Cart', 'wc_wishlist' ); ?></a>
                                            <?php elseif ( $_product->get_type() == 'external' ) : ?>
                                        <a role="button" rel="nofollow" href="<?php echo esc_url( $_product->add_to_cart_url() ); ?>" rel="nofollow" class="single_add_to_cart_button btn alt"><?php echo esc_html( $_product->single_add_to_cart_text() ); ?></a>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                        <a role="button" rel="nofollow" href="<?php echo woocommerce_wishlist_url_item_remove( $wishlist->id, $wishlist_item_key ); ?>" class="wlconfirm btn alt" title="<?php _e( 'Remove this item from your wishlist', 'wc_wishlist' ); ?>" data-message="<?php esc_attr( _e( 'Are you sure you would like to remove this item from your list?', 'wc_wishlist' ) ); ?>">Delete</a>
                                        <?php $lists = WC_Wishlists_User::get_wishlists(); ?>
                                        <?php if ( $lists && count( $lists ) > 1 ) : ?>
                                            <div class="select-url-wrapper select-move-wrapper">
                                            <img class="select-move-loader" src="<?php echo get_bloginfo('template_directory'); ?>/app/assets/img/loader.gif" alt="Loading Icon"/>
                                                <select class="select-url select-styled select-move">
                                                        ...
                                                        <?php if ( $list->id != $wishlist->id ) : ?>
                                                            <?php 
                                                                $manage_query = array(
                                                                    ...
                                                                    'wlitem' => array($wishlist_item_key),
                                                                    'wlmovetarget' => '0',
                                                                    'wlupdateaction' => $list->id,
                                                                    'wleditaction2' => $list->id,
                                                                    '_n' => wp_create_nonce( 'wc-wishlists-manage-list')
                                                                );
                                                            ?>
                                                            <option value='<?php echo json_encode($manage_query); ?>'>Move to <?php echo $list->the_title();?></option>
                                                        <?php endif; ?>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                            ...
                            <tr>
                                <td colspan="4"></td>
                                <td><a role="button" rel="nofollow" href="<?php echo woocommerce_wishlist_url_add_all_to_cart( $wishlist->id, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="btn alt wl-add-all"><?php _e( 'Add All To Cart', 'wc_wishlist' ); ?></a></td>
                            </tr>
                            </tbody>
                        </table>
				...
				<?php else : ?>
                    ...
                    <a role="button" href="<?php echo $shop_url; ?>"><?php _e( 'Go Shopping', 'wc_wishlist' ); ?></a>.
