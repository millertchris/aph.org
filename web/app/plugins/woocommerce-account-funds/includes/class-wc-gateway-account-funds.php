...
class WC_Gateway_Account_Funds extends WC_Payment_Gateway {
	...
	public function payment_fields() {
		...
		do_action( 'woocommerce_account_funds_form_start', $this->id );

		// I recommend to use inique IDs, because other gateways could already use #ccNo, #expdate, #cvc
		$account_label = 'Choose an FQ Account';
		$current_user = wp_get_current_user();
		if(in_array('net_30', $current_user->roles)){
			$account_label = 'Choose a Net 30 Account';
		}

		echo '<div class="form-row form-row-first">
				<label>'.$account_label.' <span class="required">*</span></label>
	...
	public function process_payment( $order_id ) {
		...
		$available_funds = WC_Account_Funds::get_account_funds( $order->get_user_id(), false, $order_id, $term_id );
		...
		WC_Account_Funds::remove_funds( $order->get_user_id(), $order->get_total(), $term_id );
		$order->update_meta_data( '_funds_used', $order->get_total() );
		$order->update_meta_data( '_funds_removed', 1 );
		$order->update_meta_data( '_eot_id', $order->get_user_id() );
		$order->update_meta_data( '_fq_account', $term_id );
		// Add account name as meta data
		$fq_account_name = get_term_by('term_taxonomy_id', $term_id);
	...
	public function set_renewal_order_meta( $renewal_order ) {
		...
		$renewal_order->update_meta_data( '_funds_used', $renewal_order->get_meta( '_order_total' ) );
		$renewal_order->save();
	...
	public function subscription_payment_method_name( $payment_method_to_display, $subscription_details, $order ) {
		...
		if ( $this->id !== $order->get_meta( '_recurring_payment_method' ) || ! $customer_user ) {
