...
class SearchWP_Metrics {
	...
	function is_user_blocklisted() {
		...
		$user_ip = isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : false;
	...
	function assets( $hook ) {
		...
		$settings = [
			...
			'clear_data_interval'        => [
				...
				'label' => $is_clear_data_interval_custom ? $this->i18n->strings['clear_metrics_interval_custom'] : $this->i18n->strings[ 'clear_metrics_interval_' . $clear_data_interval ],
			],
			'clear_data_interval_custom' => $is_clear_data_interval_custom ? $clear_data_interval : 0,
			'view_background_process'    => $this->get_boolean_option( 'view_background_process' ),
		];
		...
		wp_localize_script(
			...
			[
				...
				'i18n'              => $this->i18n->strings,
				'options'           => array_merge( $this->i18n->options, $this->get_min_year() ),
				'engines'           => $engines,
	...
	private function get_min_year() {
		...
		// If there is no data, return the current year.
		if ( empty( $min_year ) ) {
			$min_year = gmdate( 'Y' );
		}
	...
	public function view() {
		...
		<div class="searchwp-metrics wrap">
			<div class="searchwp-metrics__title">
				...
				<div id="searchwp-metrics__menu" class="searchwp-metrics__popover-menu">
					<button class="button"><span class="dashicons dashicons-menu"></span></button>
				</div>
			</div>

			<div class="searchwp-metrics__deletion-progress-wrap"></div>
