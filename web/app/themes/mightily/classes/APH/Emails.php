...
class Emails
{
    ...
    static function maybe_add_quote_content($order, $sent_to_admin, $plain_text, $email){
        if($order->get_status() == 'quote'){ ?>
            <p>To confirm your quote, please submit a Purchase Order referencing the above quote number to <a href="mailto:support@aph.org">support@aph.org</a>, or contact APH Customer Service to pay by credit card. For this or other questions regarding this quote, please contact APH Customer Service at 1-800-223-1839 between 8am and 8pm EST Monday through Friday.</p>
            <p>Pricing is valid for the next 30 days. All sales are in USD. Customer is responsible for import fees, taxes, and duties.</p>
        <?php }
    }
    
    // =========================================================================
    // SEND LOUIS DOWNLOAD EMAIL TO SECONDARY EMAIL ADDRESS AUTOMATIC TRIGGER
    // =========================================================================
    static function maybe_send_louis_download_email_automatic($order_id){
        $order = wc_get_order($order_id);
        $louis_download_email = $order->get_meta('_louis_download_email');
        $louis_download_email_sent = $order->get_meta('_louis_download_email_sent');
        if($louis_download_email && $louis_download_email != ''){
            // If email has been sent, dont send again
            if($louis_download_email_sent != '1'){
                // if($order->get_status() == 'processing'){
                //     $emails = wc()->mailer()->emails;
                //     $wc_processing_email = new $emails['WC_Email_Customer_Processing_Order'];
                //     $wc_processing_email->trigger($order->get_id(), false, $louis_download_email);                    
                // }
                $emails = wc()->mailer()->emails;
                // Access the default subject for new order email notifications
                $wc_processing_email = new $emails['Secondary_Louis_Download_Email'];
                $wc_processing_email->trigger($order->get_id(), $order, $louis_download_email);
                // Add order meta so we know the secondary email has been sent. We don't want to send duplicates.
                update_post_meta($order_id, '_louis_download_email_sent', '1');
            }
        }
    }    
