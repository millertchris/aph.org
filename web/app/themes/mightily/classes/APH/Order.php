...
class Order {
	...
	function getFQAName()
	{
		return $this->order->get_meta('_fq_account_name');
	}

	function getEOT()
	{
		$eot_id = $this->order->get_meta('_eot_id');
		if ($eot_id) {
			return get_userdata($eot_id);
		} else {
			return null;
	...
	function setEOT($eot_id)
	{
		$this->order->update_meta_data('_eot_id', $eot_id);
		$this->order->save();
	}

	function getFQAccountId()
	{
		return $this->order->get_meta('_fq_account');
	}

	function setFQAaccountId($value)
	{
		$this->order->update_meta_data('_fq_account', $value);

		// Set the Name for the account.
		$fq_account = get_term_by('term_taxonomy_id', $value);
		$this->order->update_meta_data('_fq_account_name', $fq_account->name);
		$this->order->save();
	}
	...
	function getSysproId() {
		...
		$sysproId = $this->order->get_meta(self::SysproOrderNumber);
		if($sysproId){
			return $sysproId;
		} else {
			return '--';
	...
	function setSysproId($value)
	{
		$this->order->update_meta_data(self::SysproOrderNumber, sanitize_text_field($value));
		$this->order->save();
		return $value;
	}
	...
	static function created_by_add_to_rest($response, $post, $request){
		// $created_by = get_post_meta($post->get_id(), '_created_by');
        // if($created_by && is_array($created_by)){
			// $response->data["created_by"] = $created_by[0];
        // } else {
			// $response->data["created_by"] = null;
		// }
		$uid = $post->get_user();
		$response->data["created_by"] = $uid->user_email;		
        return $response;
	}	
	...
	static function assign_fs_license($fs_data, $order){
        ...
        if($httpcode == '200'){
            ...
            $order_meta = 'Response code ' . $httpcode . '. Error delegating license from Freedom Scientific.';
			if($result != ''){
				$order_meta .= ' Error message: ' . $result;
			}

        }
        $existing_order_meta = get_post_meta($order->get_id(), '_fs_license_requested', true);
		if($existing_order_meta != ''){
			$existing_order_meta .= ', ';
		}
        $new_order_meta = $existing_order_meta . $order_meta;
        add_post_meta($order->get_id(), '_fs_license_requested', $new_order_meta, true);
        $order->add_order_note(__($order_meta));
		// Delay to help circumvent bug that occurs when order status change rapidly.
		sleep(2);
	}
        ...
        if(get_post_meta($order_id, '_fs_license_requested', true) == ''){
            ...
            foreach($order->get_items() as $item_id => $item){
                ...
                if(get_field('fs_product_id', $product_id) && get_field('fs_product_id', $product_id) != ''){
					// Set order number string to contain po number if it is set.
					$po_number = get_post_meta( $order->get_order_number(), 'PO Number', true );			
					if($po_number){
						$order_number = $order_id . ' and your Purchase Order Number is ' . $po_number;
					} else {
