<?php
    ...
    function get_image_data($post, $attr){
        ...
        if($attachment_url){
            if($attachment_url == 'http://nyc3.digitaloceanspaces.com/aph/app/uploads/2018/10/26162145/Product-Image-Placeholder.jpg' || $attachment_url == 'http://nyc3.digitaloceanspaces.com/aph/app/uploads/2018/10/26162145/Product-Image-Placeholder-300x225.jpg' || $attachment_url == 'https://nyc3.digitaloceanspaces.com/aph/app/uploads/2018/10/26162145/Product-Image-Placeholder-300x225.jpg'){
                return ($attr == 'url') ? $placeholder_url : 'Missing Image Placeholder';
            } else {
                return ($attr == 'url') ? $attachment_url : $attachment_alt;
            }
        } else {
            return ($attr == 'url') ? $placeholder_url : 'Missing Image Placeholder';
        }
    ...
    function get_product_image_data($product, $attr){
        $placeholder_url = get_stylesheet_directory_uri() . '/app/assets/img/aph-placeholder.png';
    ...
    // Accepts array from acf file field
    function getFileExtension($file){
        $subtype = isset($file['subtype']) ? $file['subtype'] : false;
        $inferred_extension = substr($file['filename'], strrpos($file['filename'], '.') + 1);
        // if inferred file extension from the filename is 3 or 4 characters long, use it
        if(strlen($inferred_extension) == 3 || strlen($inferred_extension) == 4){
