...
class ASPWC_Condition {
	...
	public function __construct( $id = null, $group = 0, $condition = null, $operator = null, $value = null ) {
		...
		$this->id        = absint( $id );
		$this->group     = absint( $group );
		$this->condition = wpc_clean( $condition );
		$this->operator  = wpc_clean( $operator );
		$this->value     = wpc_clean( $value );
	...
	public function output_condition_row() {
		...
		$wp_condition = $this;
		require plugin_dir_path( __FILE__ ) . 'views/html-admin-page-package-conditions-row.php';
