...
if ( class_exists('PMUE_Plugin') and PMUE_EDITION == "free"){
	...
	if( !empty($wpae_user_addon_options['info_api_url_new'])){
		$api_url = $wpae_user_addon_options['info_api_url_new'];
	}elseif( !empty($wpae_user_addon_options['info_api_url'])){
		$api_url = $wpae_user_addon_options['info_api_url'];
	}else{
		$api_url = null;
	}
	...
	if (!empty($api_url)){
		// setup the updater
		$updater = new PMUE_Updater( $api_url, __FILE__, array(
