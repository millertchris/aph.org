...
class WO_Server {
	...
	function __construct() {
		...
		if ( ! defined( 'WOCHECKSUM' ) ) {
			define( 'WOCHECKSUM', '72144D3340A3ACFF826C63374C55B2AD' );
		}
		...
		add_filter( 'determine_current_user', array( $this, 'wp_oauth_authenticate_bypass' ), 20 );
	 ...
	 */
	public function wp_oauth_authenticate_bypass( $user_id ) {

		// Needed to prevent the grant type client credentials with no user id
		if ( $user_id && $user_id > 0 ) {
			return (int) $user_id;
		}

		// @todo Should be able to remove this since the check above does this check as well.
		if ( ! empty( $user_id ) ) {
			return $user_id;
		}

		$token = wp_oauth_get_authentication_token( $_SERVER );
		$user  = wo_public_get_access_token( $token );
		if ( ! empty( $user['user_id'] ) && $user['user_id'] > 0 ) {

			// @since 4.2.1 An action to hook into an authenticated request
	...
	public function wpoauth_block_unauthenticated_rest_requests( $result ) {
		...
		$block_all_unauthenticated_rest_requests = wo_setting( 'block_all_unauthenticated_rest_request' );
		if ( ! $block_all_unauthenticated_rest_requests ) {
			return $result;
		}
