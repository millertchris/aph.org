...
class Admin extends View {
	...
	public function common_vars( $vars ) {
		...
		if ( is_admin() ) {
			...
			if ( class_exists( 'WPMUDEV_Dashboard' ) && isset( \WPMUDEV_Dashboard::$whitelabel ) ) {
				$whitelabel_settings = \WPMUDEV_Dashboard::$whitelabel->get_settings();
				// Beehive's WDP ID is 51.
				if ( ! empty( $whitelabel_settings['enabled'] ) && ! empty( $whitelabel_settings['labels_enabled'] ) && ! empty( $whitelabel_settings['labels_config'][51] ) ) {
					$label_config = $whitelabel_settings['labels_config'][51];
					if ( ! empty( $label_config['name'] ) ) {
						$labels_config['name'] = $label_config['name'];
					}
					if ( ! empty( $label_config['icon_type'] ) ) {
						$labels_config['icon_type'] = $label_config['icon_type'];
					}
					if ( ! empty( $label_config['icon_class'] ) ) {
						$labels_config['icon_class'] = $label_config['icon_class'];
					}
					if ( isset( $label_config['thumb_id'] ) ) {
						$labels_config['thumb_id'] = $label_config['thumb_id'];
					}
					if ( isset( $label_config['icon_url'] ) ) {
						$labels_config['icon_url'] = $label_config['icon_url'];
					}
				}
			}
