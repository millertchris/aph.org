...
class Fields
{
    ...
    static function get_fq_user_list($current_user) {
        ...
        foreach (wp_get_terms_for_user($current_user, 'user-group') as $group) {
            ...
            foreach (get_objects_in_term($group->term_id, 'user-group') as $child_user_id) {
                ...
                if ($child_user_id != $current_user->ID && (in_array('teacher', $child_user_object->roles) || in_array('customer', $child_user_object->roles) || in_array('eot-assistant', $child_user_object->roles))) {
    ...
    static function checkout_field_create_order_for($fields) {
        ...
        // If user is EOT only will we show this option. Does not apply to EOT Assistant
        if (Roles::userHas([Roles::TVI, Roles::EOT, Roles::OOA], $current_user) && !Roles::userHas(Roles::NET, $current_user)) {
