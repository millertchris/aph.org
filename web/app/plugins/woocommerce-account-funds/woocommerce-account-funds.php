<?php
/*
 * Plugin Name: Woocommerce EOT Account Funds
...
if (is_woocommerce_active()) {
	...
	define('WC_ACCOUNT_FUNDS_VERSION', '99.99.99');
	...
	class WC_Account_Funds {
		 ...
		 * @return string
		 */
		public static function get_account_funds($user_id = null, $formatted = true, $exclude_order_id = 0, $term_id = null) {
			$user_id = $user_id ? $user_id : get_current_user_id();

			if ($user_id) {
				// Get ID of group the user is in
				$user_terms_result = wp_get_terms_for_user($user_id, 'user-group');

				// CRITICAL FIX: Check if $user_terms_result is a WP_Error object or empty
				if (! is_wp_error($user_terms_result) && ! empty($user_terms_result)) {
					$user_terms = $user_terms_result;
					// Get the available funds value from the custom field
					$fq_available = get_field('fq_account_balance', 'user-group_' . $user_terms[0]->term_id);
					if (! is_null($term_id)) {
						$fq_available = get_field('fq_account_balance', 'user-group_' . $term_id);
					}
				} else {
					// Handle the case where no terms are found or an error occurred
					$fq_available = 0;
				}

				$funds = max(0, $fq_available);
		 ...
		 * @param float $amount
		 */
		public static function add_funds($customer_id, $amount, $term_id = null) {
			// Get ID of group the user is in
			$user_terms_result = wp_get_terms_for_user($customer_id, 'user-group');
			...
			if (! is_wp_error($user_terms_result) && ! empty($user_terms_result)) {
				...
				// If no terms or error, default to 0 for fund calculation
				$fq_available = 0;
			}

			// If group ID is present, use balance of group instead of first term attached to user
			if (! is_null($term_id)) {
				$fq_available = get_field('fq_account_balance', 'user-group_' . $term_id);
			}
			$funds = $fq_available;
			...
			if (! is_null($term_id)) {
				update_field('fq_account_balance', $funds, 'user-group_' . $term_id);
			} else {
				// Only update if $user_terms was successfully retrieved and is not empty
				if (isset($user_terms) && ! empty($user_terms)) {
					update_field('fq_account_balance', $funds, 'user-group_' . $user_terms[0]->term_id);
				}
			}
		}
		 ...
		 * @param float $amount
		 */
		public static function remove_funds($customer_id, $amount, $term_id = null) {
			// Get ID of group the user is in
			$user_terms_result = wp_get_terms_for_user($customer_id, 'user-group');
