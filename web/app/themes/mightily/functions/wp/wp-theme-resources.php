...
function mightyResources() {
    ...
    wp_enqueue_style('mightily-css', get_stylesheet_directory_uri() . '/app/assets/css/style.min.css', '', '6.6.5');
    ...
    wp_enqueue_script('mightily-js', get_stylesheet_directory_uri() . '/app/assets/js/scripts.min.js', ['jquery'], '6.6.5', true);
    ...
    if (is_checkout()) {
        ...
        wp_enqueue_script('google-maps', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyDnmYbN_lbeH6pp9LRqUunYOxckiX1njmg&libraries=places&callback=initAutocomplete', '', '', true);
    }
    // Manage Addresses Scripts
    if (\APH\Addresses::is_address_enabled()) {
        wp_enqueue_script('babel-polyfill', get_stylesheet_directory_uri() . '/app/assets/js/babel.polyfill.min.js', '', '6.6.5', false);
        wp_enqueue_script('vue', 'https://unpkg.com/vue@2.6.11/dist/vue.js', '', '', false);
        wp_enqueue_script('vue-carbon', get_stylesheet_directory_uri() . '/app/assets/js/carbon.vue.min.js', '', '6.6.5', false);
    }
